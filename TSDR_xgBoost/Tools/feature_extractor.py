...
def extractFeatures(img):
    ...
    if 'GRAYPATCH' in cfg.featuresToExtract:
        graypatchFeats = extractGRAYPATCHfeatures(img)
        feats.extend(graypatchFeats)
    if 'HAAR' in cfg.featuresToExtract:
        haarFeats = extractHAARfeatures(img)
        feats.extend(haarFeats)	
    return feats
...
def extractGRAYPATCHfeatures(img):
	return img.ravel() #Returns the input grayscale image as a 1D vector.
	
def extractHAARfeatures(img):
	cA, cD = pywt.dwt(img, 'Haar')
	fd = cA.flatten() #To 1D array
	return fd